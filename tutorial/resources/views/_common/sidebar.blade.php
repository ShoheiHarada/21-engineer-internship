    ...
    <a href="/mypage" class="text-dark text-decoration-none w-100 h-100">
      <div class="card-header p-1">
        <h5 class="font-weight-bold m-0">
          ...
          <img class="rounded-circle" src="/storage/user_image/user_{{\Auth::guard('user')->Id()}}.jpg" alt="プロフィール画像"
  ...
  <div class="p-3">
    <form method="get" action="/">
      <div class="input-group w-100">
        <input type="search" class="rounded w-75" name="search" value="@if(isset($search)){{$search}}@endif"
          placeholder="検索">
    ...
    <table class="mb-0 table-bordered table-striped w-100">
      <thead>
        <tr>
          <th class="small" width="30px">順位</th>
          <th class="small">ルーム名</th>
          <th class="small" width="70px">ユーザー数</th>
        </tr>
      </thead>
      <tbody>
        @php($i = 1)
        @foreach($user_ranking as $rank)
        <tr>
          <td class="text-center">{{ $i }}</td>
          <td>
            <a href="/room?room_id={{ $rank['room_id'] }}" title="{{$rank['title']}}">
          ...
          <td class="text-center">{{ $rank['user_count'] }}</td>
        </tr>
        @php( $i++ )
        @endforeach
      </tbody>
