...
<div class="row">
  <div class="col-md-8">
    <div>
      <a href="/category/detail?category_id={{$room_data['category_id']}}"
        class="mb-2 text-primary font-weight-bold">{{ $room_data['category_name'] }}</a>
      <h2>{{ $room_data['title'] }}
        @if(file_exists("storage/room_image/room_$room_id.jpg"))
        <img class="rounded-circle" src="/storage/room_image/room_{{$room_data['room_id']}}.jpg" alt="room画像"
          width="200px" height="200px">
        @endif
        <span class="text-muted float-right">
          @if(count($favorite_room)==0)
          <form method="post" action="/room/newfavorite">
            {{ csrf_field() }}
          ...
          <form method="post" action="/room/favorite">
            {{ csrf_field() }}
            <input type="hidden" name="room_id" value="{{ $room_data['room_id'] }}">
            <input type="submit" value="お気に入り">
          </form>
          @else
          <form method="post" action="/room/unfavorite">
            {{ csrf_field() }}
            <input type="hidden" name="room_id" value="{{ $room_data['room_id'] }}">
        ...
        @if ($room_data['can_edit']||$user_flag['user_flag1']==1)
        <span class="text-muted float-right">
      ...
      <p>作成日:{{ date('Y年m月d日 H:i:s', strtotime($room_data['created_date'])) }}　作成者 :
        {{ $room_data['creator_unsub']? '[退会したユーザー]' : $room_data['creator']}}</p>
    ...
    <div>
      ...
      <div class="card mb-1">
        <div class="card-body align-items-start">
          ...
          <div class="mt-n3 text-muted">{{ $comment['destination_name'] }}への返信<i class="fa fa-fw fa-reply"></i>
          </div>
          ...
          <a href="/user?user_id={{$comment['user_id']}}" class="mb-2 text-primary font-weight-bold">
            ...
            <img class="rounded-circle" src="/storage/user_image/user_{{$comment['user_id']}}.jpg" alt="プロフィール画像"
          ...
          <span class="text-muted float-right">
            ...
            <a href="#" class="fa fa-fw fa-reply text-success text-decoration-none" data-toggle="modal"
              data-target="#replyModal{{$comment['comment_id']}}" Id="reply"></a>
            <!--返信用モーダル-->
            <div class="modal fade" id="replyModal{{$comment['comment_id']}}" tabindex="-1" role="dialog"
                ...
                <div class="modal-content">
                  ...
                  <div class="modal-body">
                    <form method="post" action="room/comment">
                      ...
                      <textarea class="w-100" name="comment_body" maxlength="500" placeholder="コメントを記入"
                        rows="5"></textarea>
            ...
            @if($user_flag['user_flag1']==1)
            <a href="#" class="fa fa-fw fa-trash text-muted text-decoration-none" data-toggle="modal"
              data-target="#deleteModal{{$comment['comment_id']}}"></a>
            <div class="modal fade" id="deleteModal{{$comment['comment_id']}}" tabindex="-1" role="dialog"
            ...
            @endif
            @elseif($comment['can_edit']||$user_flag['user_flag1']==1)
            <a href="#" class="fa fa-fw fa-pen text-primary text-decoration-none" data-toggle="modal"
              data-target="#editModal{{$comment['comment_id']}}"></a>
            <a href="#" class="fa fa-fw fa-trash text-muted text-decoration-none" data-toggle="modal"
              data-target="#deleteModal{{$comment['comment_id']}}"></a>
            <!--編集用モーダル-->
            <div class="modal fade" id="editModal{{$comment['comment_id']}}" tabindex="-1" role="dialog"
                ...
                <div class="modal-content">
                  ...
                  <div class="modal-body">
                    ...
                    <form method="post" action="room/comment/edit">
                      ...
                      <input type="hidden" name="comment_id" value="{{ $comment['comment_id'] }}">
                      <input type="hidden" name="room_id" value="{{ $room_data['room_id'] }}">
                      <textarea class="w-100 my-3 rounded" name="comment_body" maxlength="500" placeholder="コメントを記入"
                        rows="5">{{ $comment['comment_body'] }}</textarea>
                      <div class="d-flex justify-content-end">
                        <input type="submit" class="btn btn-primary px-4">
                      </div>
                    </form>
          ...
          <p class="card-text mb-auto font-weight-bold">
    ...
    <div class="alert-secondary p-4 border rounded shadow-sm mb-3" id="comment">
      <form method="post" action="room/comment">
        ...
        <input type="hidden" name="room_id" value="{{ $room_data['room_id'] }}">
        <textarea class="w-100" name="comment_body" maxlength="500" placeholder="コメントを記入"
          rows="5">{{ old('comment_body') }}</textarea>
        <div class="d-flex justify-content-end">
    ...
    <div class="p-3 mb-3 alert-dark rounded">
      ...
      <p class="mb-0 p-3">アカウントを作成しログインすると、ルームを作成したり、ルームにコメントすることができます。<br>アカウントの作成は、<a href="/sign-up">こちら</a>から
      </p>
    </div>
    @endif
  </div>
    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
        <form method="post" action="/room/edit">
          {{ csrf_field() }}
          <input type="hidden" name="room_id" value="{{ $room_data['room_id'] }}">
          <input type="text" name="title" class="w-100 my-3" maxlength="100" minlength="1" placeholder="ルーム名を記入"
            value="{{ $room_data['title'] }}">
          <input type="text" name="category_name" class="w-100 my-3" maxlength="50" minlength="1" placeholder="ルーム名を記入"
            value="{{ $room_data['category_name'] }}">
          <textarea class="w-100 my-3 rounded" name="body" maxlength="500" minlength="1" placeholder="ルーム詳細を記入"
            rows="5">{{ $room_data['body'] }}</textarea>
          <div class="d-flex justify-content-end">
            <input type="submit" class="btn btn-primary px-4">
          </div>
        </form>
